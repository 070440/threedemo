  ...
  methods: {
    init: function () {
      ...
      let distance2 = Math.sqrt(
        (point2.x - point4.x) * (point2.x - point4.x) +
          (point2.y - point4.y) * (point2.y - point4.y)
      );
      let scale1, scale2, shape, uv = [];
      shape = new THREE.Shape();
      console.log(distance1, distance2);
      if (distance1 > distance2) {
        //2,4号节点在内侧
        point5.x = ((b2 - b4) * 1.0) / (k1 - k2);
        point5.y = k2 * point5.x + b2;
        ...
        shape.lineTo(point5.x, point5.y);
        // shape.arc(0, 0, 10,  Math.PI, 2 * Math.PI);
        shape.lineTo(point4.x, point4.y);
        shape.lineTo(point3.x, point3.y);
        shape.lineTo(point6.x, point6.y);
        shape.lineTo(point7.x, point7.y);
        
        
      } else {
        //1,3号节点在内侧
        point5.x = ((b1 - b3) * 1.0) / (k1 - k2);
        point5.y = k1 * point5.x + b3;
        point6.x = -point3.x + point5.x + point4.x;
        point6.y = -point3.y + point5.y + point4.y;
        point7.x = -point1.x + point5.x + point2.x;
        point7.y = -point1.y + point5.y + point2.y;

        scale1 = this.distance(point1, point5) / width;
        scale2 = this.distance(point3, point5) / width;
      ...
      shapeGeometry.attributes.uv = new THREE.Float32BufferAttribute(uv, 2);
      ...
      let line1 = new THREE.Mesh(shapeGeometry, material);
